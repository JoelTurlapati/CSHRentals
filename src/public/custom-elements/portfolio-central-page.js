...
class CshPortfolioCentral extends HTMLElement {
  connectedCallback() {
    ...
    var PORTFOLIO = 'Portfolio 2 — Hall of Fame Village / Malone University #2';
...
`;
    ...
    if (plDlBtn)   plDlBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      if (_currentPlYear) downloadCSV('Portfolio2_HallOfFameVillage_Malone2_PL_' + _currentPlYear + '.csv', buildPLCsv(_currentPlYear));
    });
    ...
    if (rrDlBtn)   rrDlBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      downloadCSV('Portfolio2_HallOfFameVillage_Malone2_RentRoll.csv', buildRRCsv());
    });
